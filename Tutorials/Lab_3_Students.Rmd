---
title: "EEMB 179, Lab 3 -- Optimal Harvest"
...
## Modelling using a given set of parameters
...
#################################
tset <- seq(from = 0, to = 100, length.out = 20000) #creating tset which is a vector of length 20,000 with values from 0-100


#################################
# 2. create a holding vector
#################################
N.simu <- NaN * tset # multiplying NaN by tset automatically creates a vector the same length as tset, but only containing NaN values
# same as N.simu <-  rep(NaN, length= tset)
N0 <- 2 # initial population size
N.simu[1] <- N0 # assigning first vlaue of N simulation vector equal to NO, which is 2 individuals

#################################
# 3. the for() loop
#################################
#
for(i in 2:length(tset)){ #offset length of vector that we iterate over in for loop to 2
  # calculating the change in t, which is dt
  dt <- tset[i] - tset[i-1] 
  
  # calculating the rate of population change, which is dN/dt
  dNdt <- (r * N.simu[i-1] * (1-N.simu[i-1]/K)) * dt 
  
  #calculate the next N size using the previous N size and the rate of popn growth
...
```{r}
plot(x = tset, y = N.simu,  # x axis is time, tsetm y axis is N simu, simulated population size
     type = 'l', 
     ylim = c(0, 1.1*K), 
     las = 1, 
     lwd = 2, 
...
for(i in 2:length(tset)){
  ...
  ##calculating change in pop size for 50 starting individuals
  dNdt.50 <- (r*N.simu.50[i-1]*(1-N.simu.50[i-1]/K))*dt
  N.simu.50[i] <- N.simu.50[i-1] + dNdt.50
  
  ##calculating change in pop size for 100 starting individuals
  dNdt.100 <- (r*N.simu.100[i-1]*(1-N.simu.100[i-1]/K))*dt
  N.simu.100[i] <- N.simu.100[i-1] + dNdt.100
  
  #calculating change in pop size for 150 starting individuals
...
# it looks exactly like the previous graph we made for N0=2
######################################################
...
# x and y in legend is position
# legend= c( is character strings
# col= c is color of lines to attach to each charcter string
#horiz= FALSE makes legend vertical
```

## dN/dt as a function of N
...
#geenrating our population sizes
```
...
```{r}
# standard logistic growth equation, we already defined K and r
tset <- seq(from = 0, to = 100, length.out = 20000) 
Nset <- seq(from = 0, to = K*1.1, length.out = 200)
dNdt.log <- r * Nset * (1-Nset/K)
# calculate dn/dt with a vector equation, but we could do it with a for loop as well
```
...
## Population change as a function of population size
...
# 2. create a holding vector
#################################
N.simu.E1 <- NaN*tset #create holding vector for simulation pop values
N.simu.E1[1] <- N0 #store N0=2 as initial

#################################
# 3. the for() loop
#################################
for(i in 2:length(tset)){ #offsetting our length by one to accomodate current and previous time points
  dt <- tset[i] - tset[i-1] #calc time points
  dNdt.E1 <- (r*N.simu.E1[i-1]*(1-N.simu.E1[i-1]/K)-q*E*N.simu.E1[i-1])*dt #calc pop growth rate dN/dt with logistic growht equation
  N.simu.E1[i] <- N.simu.E1[i-1] + dNdt.E1 # creating the next population size
}
```
...
Increasing E means that yield increases more rapidly over shorter changes in population size at large population sizes, but when effort is too high it doesn't give the populatio time to grow and the yield begins to lower again.
If effort was below one, yield would show up as negative, which would indicat e yield of zero
...
Eset <- seq(from = 0, to = 5 , length.out = 200)
```
...
# Part Four: Calculating Maximum Sustainable Yield (MSY)
...
## Comparing logistic and overcompensating growth
...
No the equilibria do not differ, but the time to get to the vlaue differs because overcompensating species increase at a faster pop growth rate. so for each time step, the population increase more in the overcomoensating population. However, K is still included in the additional overcompensation term, so the pop is still bounded by K (max pop size supported by available resources).
...
for(i in 1:length(Nset)){
  #when the ith value of change in pop equal yield of over compensating population, store the ith value of Nset in N_MSY_over
...
Lastly, calculate effort given your values for `Y_MSY.over` and `N_MSY.over`. Recall that $Y = qEN$.
```{r}
E_MSY.over <- Y_MSY.over/(N_MSY.over*q)
```

Now that you're done, check your results by making a graph of population change due to biology (in black) and economics (in blue) as a function of population size. Fill in the axis blanks.
```{r}
###################################
# 1. biology
###################################
